<template>
  <div class="merchant-management">
    <el-card>
      ...
      <el-table :data="merchantList" v-loading="loading" border @row-click="handleRowClick">
    ...
    >
      ...
      >
        ...
        <el-form-item label="商家等级" prop="merchantLevel">
          <el-input-number v-model="editForm.merchantLevel" :min="1" :max="5" />
        </el-form-item>
        <el-form-item label="手续费率" prop="merchantFee">
          <el-input-number 
            v-model="editForm.merchantFee" 
            :min="0" 
            :max="1" 
            :step="0.01" 
            :precision="2"
      ...
      <div class="image-section">
        ...
        <div class="image-grid">
          ...
          <div class="image-item">
            <h4>身份证照片</h4>
            <el-image
              v-if="editForm.idCardImage"
              :src="`http://localhost:8080${editForm.idCardImage}`"
              :preview-src-list="[`http://localhost:8080${editForm.idCardImage}`]"
              fit="cover"
          ...
          <div class="image-item">
            <h4>营业执照照片</h4>
            <el-image
              v-if="editForm.businessLicenseImage"
              :src="`http://localhost:8080${editForm.businessLicenseImage}`"
              :preview-src-list="[`http://localhost:8080${editForm.businessLicenseImage}`]"
              fit="cover"
              style="width: 200px; height: 120px"
            />
            <span v-else>暂无</span>
...
const fetchMerchantList = async () => {
  try {
    ...
    console.log('【MerchantManagement】获取用户列表响应:', res)
    // 直接使用返回的数组数据
    if (Array.isArray(res)) {
      // 只显示商家用户（userType === 1）且不是管理员（role !== 2）
      const filteredMerchants = res.filter(user => user.userType === 1 && user.role !== 2)
      console.log('【MerchantManagement】过滤后的商家列表:', filteredMerchants)
      merchantList.value = filteredMerchants
    } else {
      console.error('【MerchantManagement】获取用户列表失败: 数据格式错误')
